...
curl -o Python-$pyver.$pyrel.tar.xz https://www.python.org/ftp/python/$pyver.$pyrel/Python-$pyver.$pyrel.tar.xz

# %prep
tar -jxf Python-$pyver.$pyrel.tar.xz
cd Python-$pyver.$pyrel
cp ../../python-2.6-fix-cgi.patch ../.
...
# We had installed openssl=1.0.2l in opt/local folder
make -j2
